...
class HomeEndDrawerV3PanelSections extends ConsumerStatefulWidget {
  const HomeEndDrawerV3PanelSections({super.key});

  @override
  ConsumerState<HomeEndDrawerV3PanelSections> createState() =>
      _HomeEndDrawerV3PanelSectionsState();
}

class _HomeEndDrawerV3PanelSectionsState extends ConsumerState<HomeEndDrawerV3PanelSections> {
  _TopKey selected = _TopKey.sermon;

  // ✅ V3: 우측 섹션 펼침/접힘
  bool openLiveGroup = true;
  bool openPastGroup = true;
    ...
    switch (selected) {
      case _TopKey.sermon:
        return [
          _RightSectionHeader(
            title: '유튜브 실시간 생방송',
            isOpen: openLiveGroup,
            onToggle: () => setState(() => openLiveGroup = !openLiveGroup),
            onTapTitle: () {
              Navigator.pop(context);
              showModalBottomSheet(
                ...
                builder: (_) => const SermonSelectSheet(), // 하단메뉴1/2 공용
              );
            },
          ),
          if (openLiveGroup)
          ...
          _RightSectionHeader(
            title: '지난 설교 말씀',
            isOpen: openPastGroup,
            onToggle: () => setState(() => openPastGroup = !openPastGroup),
            onTapTitle: () {
              Navigator.pop(context);
          ...
          if (openPastGroup) ...[
            _RightSubItem(
              ...
              onTap: () async {
                Navigator.pop(context);
                await _launchExternal('${AppLinks.homepage}/sermon/sunday'); // TODO 실제 URL로 교체
              },
            ),
            _RightSubItem(
              title: '부교역자 설교',
              ...
              onTap: () async {
                Navigator.pop(context);
                await _launchExternal('${AppLinks.homepage}/sermon/staff'); // TODO 실제 URL로 교체
              },
            ),
            _RightSubItem(
              title: '초청강사설교',
              ...
              onTap: () async {
                Navigator.pop(context);
                await _launchExternal('${AppLinks.homepage}/sermon/guest'); // TODO 실제 URL로 교체
              },
            ),
            _RightSubItem(
              title: '특별집회',
              ...
              onTap: () async {
                Navigator.pop(context);
                await _launchExternal('${AppLinks.homepage}/sermon/special'); // TODO 실제 URL로 교체
              },
            ),
          ],
        ];
...
class _RightSectionHeader extends StatelessWidget {
  final String title;
  final bool isOpen;
  final VoidCallback onToggle;
  final VoidCallback onTapTitle;

  const _RightSectionHeader({
    required this.title,
        ...
        child: Row(
          children: [
            Expanded(
              child: Text(
                title,
                style: const TextStyle(fontSize: 13.8, fontWeight: FontWeight.w800),
              ),
            ),
            IconButton(
...
class _RightSubItem extends StatelessWidget {
  final String title;
  final VoidCallback onTap;
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            Icon(
              openInNew ? Icons.open_in_new : Icons.chevron_right,
              size: 16,
