...
import '../../../prayer/widgets/privacy_chip.dart';
...
class _PrayerPageState extends ConsumerState<PrayerPage> {
  ...
  Widget _buildCommunalList() {
    final async = ref.watch(commonPrayersProvider);
    return RefreshIndicator(
      onRefresh: () async => ref.invalidate(commonPrayersProvider),
      child: AsyncValueBuilder<List<Prayer>>(
        value: async,
        onRetry: () => ref.invalidate(commonPrayersProvider),
        emptyMessage: '공동기도제목이 없습니다.',
        builder: (items) {
          return ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              return _CardListTile(
                ...
                title: d.title,
                onTap: () => context.push('/prayer/detail', extra: extra),
              );
            },
          );
        },
  ...
  Widget _buildMyPrayerList() {
    ...
    final async = ref.watch(minePrayersProvider);

    return RefreshIndicator(
      onRefresh: () async => ref.invalidate(minePrayersProvider),
      child: AsyncValueBuilder<List<Prayer>>(
        value: async,
        onRetry: () => ref.invalidate(minePrayersProvider),
        // 빈 목록이어도 작성 안내/버튼은 보여줘야 해서 자체 처리
        isEmpty: (_) => false,
        builder: (all) {
